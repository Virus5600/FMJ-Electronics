							...
							<div class="card">
								...
								<div class="card-body m-5 " id="printThis">
									...
									<?php
										...
										$sql = "SELECT CAST(`t`.`date_added` AS DATE) AS `transaction_date`, `c`.`product_item_type_name` AS `item_name`, `p`.`category_Id` AS `cat_id`, (`t`.`price` * `t`.`qty`) AS `sales`, `t`.`item_code` AS `item_code` FROM `transactions_table` AS `t` INNER JOIN `products` AS `p` ON `t`.`product_Id` = `p`.`product_Id` INNER JOIN `category_product_item_type_table` AS `c` ON `p`.`type_Id` = `c`.`category_product_item_type_Id` GROUP BY `transaction_date`, `c`.`product_item_type_name` ORDER BY `transaction_date` DESC;";
										$result = $conn->query($sql);
										$results = [];
